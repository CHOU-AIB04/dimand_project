...
const Auth = () => {
  ...
  let {Formconfirmation,Handlechange,switchform,setswitchform} = useContext(Authcontext);
  const {setPersonalInfo,setUserAnoucement,setUserOffer,setClientResponse,encryptId,secretKey,setislogged,decryptId} = useContext(UserData)
    ...
    if (Formconfirmation(loginform) === 2) {
      ...
      axios.post("http://localhost/MY_PROJECTS/moto_project/auth",Form).then((res)=>{
          if (res.data === false) {
            ...
            window.sessionStorage.setItem("token",encryptId(Id.toString(),secretKey))
            setislogged(true)
            navigate("/")
            toast.success("welcome to JAWA.!!")
          }
      })
    }
  }

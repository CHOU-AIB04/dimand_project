   ...
   <>
    {
        ...
        ClientResponse.map((res)=>{
          return(
            <nav className='h-[550px] w-[350px] flex flex-col insh rounded-md'>
              <Link to={"/Details"}>
                <img src={`http://localhost/MY_PROJECTS/diamand/assets/${res.Picture}`} alt="moto" className="transition-all duration-500 hover:scale-110 cursor-pointer"/>
              </Link>
              <div className='flex flex-col gap-3 items-center'>
                ...
                <p className='text-sm text-center'>{res.Offer}</p>
                {
                  ...
                  : res.Accepted === 2
                  ?  <button  className="w-[120px] h-11 sh rounded-md  bg-red-600 text-white flex justify-center items-center cursor-default">Refuser</button>
     ...
     </section> : <h1 className='mt-10 text-center font-bold text-yellow-600 textsh'>Il'ya aucune Reponse pour l'instant</h1>
    }
   </>
  )
}
